		...
		if not hxs.select('//body[@class="guest directory"]'): #if it is not a directory (its a regular page)
			if hxs.select('//meta[@name="pageImpressionID"]'): 
				...
				if filterForUS and checkLocation(item['location']):
					...
					item['overviewCurrent'] = striplist(hxs.select('//dd[@class="summary-current"]/ul/li/text()').extract())
					item['currentPosition'] = striplist(hxs.select('//div[@class="position  first experience vevent vcard summary-current"]/div/h3/span/text()').extract())
                                        item['currentCompany'] = striplist(hxs.select('//div[@class="position  first experience vevent vcard summary-current"]/div/h4/strong/a/span/text()').extract())
					...
					item['summarySpecialties']		= striplist(hxs.select('//ol[@id="skills-list"]/li/span/text()').extract())
